...
bool checkNums(std::vector<int> numberList, std::vector<int> toCheck) {
    std::vector<int>::iterator findResult;
    int foundNumbers = 0;

    for (unsigned int i = 0; i < toCheck.size(); i++) {
        findResult = std::find(numberList.begin(), numberList.end(), toCheck.at(i));
        if (findResult != numberList.end()) {
            foundNumbers++;
        }
    }
    if (foundNumbers == toCheck.size()) {
        return true;
    } else {
        return false;
    }
